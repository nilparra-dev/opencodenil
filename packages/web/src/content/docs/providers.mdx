...
:::info
Connecting a Claude Pro/Max account makes OpenCode authenticate with the OAuth
client that ships with Claude Code. Anthropic's terms prohibit using the
consumer subscription outside its official clients, so this is unsupported:
model access can be gated on the client version, requests share the same 5-hour
and weekly usage windows as Claude Code, and Anthropic may block it at any time.
